(function() {
  var Reminder, menu_link_style, menu_template, store, template;
  ...
  Reminder = (function() {
    ...
    Reminder.prototype.locationChanged = function(evt) {
      var hash, match;
      this.detectDocument();
      if ($("#emailReminder", this.doc).length) {
        return false;
      }
      hash = document.location.hash;
      match = hash.match(/#(compose|(?:(?:inbox|draft)\/(.*)))/);
      return this.addUI(match != null ? match[2] : void 0);
    };
    Reminder.prototype.addUI = function(email_id) {
      var container, lastrow;
      console.warn(email_id);
      lastrow = $("tr.ee", this.doc);
      container = $(template);
      container.insertBefore(lastrow);
      container.find('#emailReminder-send').bind('click', __bind(function(evt) {
        if (!$(evt.currentTarget).find('.J_M').length) {
          $(menu_template).appendTo($(evt.currentTarget)).delegate("a.menu-anchor", "click", __bind(function(evt) {
            $('#conditional-caption', this.doc).html(evt.currentTarget.innerHTML);
            return $('.J-M', this.doc).hide();
          }, this)).delegate("a.show_notification", "click", __bind(function(evt) {
      ...
      return $(this.doc).bind('click', __bind(function(evt) {
        if (evt.currentTarget.nodeName.toUpperCase() === 'A' || !$(evt.currentTarget, this.doc).closest('.J-M').length) {
          return $('.J-M', this.doc).hide();
        }
      }, this));
    };
    Reminder.prototype.showNotification = function() {
      $(".b8.UC .vh", this.doc).html("Email reminder for <span class='ag ca' role='link'>mail</span>&nbsp;<span class='ag ca close' role='link'>close</span>").delegate(".close", "click", __bind(function() {
    ...
    Reminder.prototype.detectDocument = function() {
      ...
      if (this.doc.document) {
        return this.doc = this.doc.document;
      }
    };
    return Reminder;
  })();
